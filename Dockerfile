...
RUN echo 'Header always set Strict-Transport-Security "max-age=31536000"' > /etc/apache2/conf-enabled/neucore.conf && \
    echo "Header always set Content-Security-Policy \"default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' data:; font-src 'self' data:; img-src 'self' data: https://images.evetech.net; connect-src 'self' https://esi.evetech.net;\"" >> /etc/apache2/conf-enabled/neucore.conf && \
