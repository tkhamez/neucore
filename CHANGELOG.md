...
- Change: Removed two PHP object caches related to the HTTP client (also trying to fix 
  too-many-open-files errors).
...
- Change: Replaced the filesystem storage for the HTTP cache with a database storage (trying to fix
  too-many-open-files errors when `run-jobs.sh` is executed and there are many characters).
...
- New: Added a simple implementation for the new ESI rate limits.
...
- Fixed ESI errors "number of items must be less than or equal to ..." (at least one endpoint was 
  changed without increasing its version).
